...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, error = TRUE, purl=TRUE)
...
```{r peakpicking, error=TRUE, purl=TRUE}
library(xcms)
...
```{r deisoadduct, error=TRUE, purl=TRUE}
library(xcms)
...
given_stans <- read.csv("../metadata/made_data/clean_stans.csv") %>% 
  filter(polarity==!!polarity) %>% 
  filter(run_date[1]>=date_added | is.na(date_added))
...
```{r assignids, purl=FALSE}
filled_peaks <- read.csv(paste0(output_folder, "M_area_peaks.csv"))
feature_envelopes <- read.csv(paste0(output_folder, "feature_envelopes.csv"))
...
want_stans <- c("MT_POS_FT0489", "MT_POS_FT0519", "MT_POS_FT0525", "MT_POS_FT0567", 
"MT_POS_FT0584", "MT_POS_FT0720", "MT_POS_FT0862", "MT_POS_FT0973", 
"MT_POS_FT0433", "MT_POS_FT0618", "MT_POS_FT0664", "MT_POS_FT0691", 
"MT_POS_FT0744", "MT_POS_FT0607", "MT_POS_FT0617", "MT_POS_FT1113", 
"MT_POS_FT1178")
feature_data <- feature_data %>% filter(feature%in%str_remove(want_stans, "MT_POS_"))
