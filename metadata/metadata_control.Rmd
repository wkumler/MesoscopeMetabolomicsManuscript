...
if(!file.exists("raw_data/MESO_SCOPE_Cruise_KM1709_Sample_Log.xlsx")){
  stop("Go download the data from email, search 'FALKOR- Sample Log_INGALLS'")
  # Needs to be converted to .xlsx and downloaded
}
...
samp_metadata <- list.files("../mzMLs/pos", pattern = "Smp.*mzML") %>%
  data.frame(filename=.) %>%
...
extra_metadata <- list.files("../mzMLs/pos", pattern = "Blk|Std|Poo.*mzML") %>%
  data.frame(filename=.) %>%
...
if(!file.exists("made_data/file_timestamps.csv")){
  list.files("../mzMLs/pos", pattern = "mzML", full.names = TRUE) %>%
...
file_metadata <- bind_rows(samp_metadata, extra_metadata) %>%
  ...
  select(c("filename", "samp_type", "cruise", "station", "cast", "depth", 
           "abs_depth", "rosette", "tripl", "id", "untripl", "date_run", 
           "timestamp"))
...
library(GGally)
filled_MT_metadata %>%
  select(station, depth, abs_depth, time, lat, sla, local_hour, alk, dic, ph, 
         ends_with("um"), ends_with("ug"), ends_with("abund")) %>%
  ggpairs(aes(color=depth)) %>%
  ggsave(filename = "MT_ggpairsplot.pdf", device = "pdf", path = "figures", 
...
```{r pca, eval=FALSE}
filled_MT_metadata <- read_csv("made_data/filled_file_metadata.csv") %>%
  filter(cruise=="MT") %>%
  filter(samp_type=="Smp") %>%
  mutate(depth=factor(depth, levels = c("15m", "DCM", "175m")))

pca_output <- filled_MT_metadata %>%
  select(filename, abs_depth, time, lat, sla, local_hour, #alk, dic, ph, 
...
# plot(pca_output)
biplot(pca_output)
biplot(pca_output, choices = c(3, 4))

ggpca_data <- pca_output$x %>%
  as.data.frame() %>%
  rownames_to_column("filename") %>%
  select(filename, PC1:PC4) %>%
  left_join(filled_MT_metadata)
ggpca_arrows <- pca_output$rotation %>% 
  `*`(10) %>%
  as.data.frame() %>%
  rownames_to_column("env_param")
ggpca_labels <- ggpca_arrows %>% 
  mutate(PC2=PC2+0.2) %>%
  mutate(PC2=case_when(
    env_param=="sla"~PC2-0.4,
    env_param=="PC_um"~PC2+0.2,
    env_param=="phaeo_ug"~PC2+0.2,
    TRUE~PC2)) %>%
  mutate(PC4=PC4+0.3)
ggplot(mapping = aes(x=PC1, y=PC2)) + 
  geom_point(data = ggpca_data, aes(color=depth)) +
  geom_segment(data = ggpca_arrows, aes(xend=0, yend=0), 
               arrow = arrow(length = unit(0.02, "npc"), ends = "first")) +
  geom_label(data = ggpca_labels, aes(label=env_param)) +
  theme_bw() +
  theme(legend.position = c(0.8, 0.8))
ggsave(filename = "MT_PC1_PC2.png", device = "png", path = "figures", 
       width = 5, height = 5, dpi = 144, units = "in", type="cairo")
ggplot(mapping=aes(x=PC3, y=PC4)) + 
  geom_point(data=ggpca_data, aes(color=sla, size=lat)) +
  geom_segment(data = ggpca_arrows, aes(xend=0, yend=0), 
               arrow = arrow(length = unit(0.02, "npc"), ends = "first")) +
  geom_label(data = ggpca_labels, aes(label=env_param)) +
  theme_bw()
ggsave(filename = "MT_PC3_PC4.png", device = "png", path = "figures", 
       width = 5.5, height = 5, dpi = 144, units = "in", type="cairo")
```
...
clean_stans <- raw_stans %>%
  ...
  select(compound_type=Compound_Type, compound_name=Compound_Name,
         ...
         peak_quality) %>%
  add_row(compound_type="Custom", compound_name="Bisulfate",
          compound_name_old="Bisulfate", formula="HSO4", rt=13, 
          mz=95.95172965+1.007276, ionization_form="[M]", 
          charge=-1, date_added=150101, polarity="neg") %>%
  add_row(compound_type="Custom", compound_name="Bromine81",
          compound_name_old="Bromine upper", formula="Br(81)", rt=7, 
          mz=80.91563, ionization_form="[M]", charge=-1, 
          date_added=150101, polarity="neg") %>%
  add_row(compound_type="Custom", compound_name="Bromine79",
          compound_name_old="Bromine lower", formula="Br(79)", rt=7, 
          mz=78.91768, ionization_form="[M]", charge=-1, 
          date_added=150101, polarity="neg") %>%
  add_row(compound_type="Custom", compound_name="Pyro from glutamine?",
          compound_name_old=compound_name, formula="C5H7NO3", rt=11, 
          mz=130.050419, ionization_form="[M-NH3+H]", charge=-1, 
          date_added=150101, polarity="pos") %>%
  add_row(compound_type="Custom", compound_name="Pyro from glutamate?",
          compound_name_old=compound_name, formula="C5H7NO3", rt=12, 
          mz=130.050419, ionization_form="[M-H2O+H]", charge=-1, 
          date_added=150101, polarity="pos") %>%
  # add_row(compound_type="Custom", compound_name="Deoxyguanosine?",
  #         compound_name_old=compound_name, formula="C10H13N5O4", rt = 4.5,
  #         mz=268.10403, ionization_form="[M+H]", charge=1, 
  #         date_added=150101, polarity="pos") %>%
  add_row(compound_type="Custom", compound_name="Deoxycytidine?",
          compound_name_old=compound_name, formula="C10H13N5O4", rt = 6.6,
          mz=228.09788, ionization_form="[M+H]", charge=1, 
          date_added=150101, polarity="pos") %>%
  add_row(compound_type="Custom", compound_name="Homoserine betaine?",
          compound_name_old=compound_name, formula="C7H15NO3", rt = 6.7,
          mz=162.1125, ionization_form="[M+H]", charge=1, 
          date_added=150101, polarity="pos") %>%
  add_row(compound_type="Custom", compound_name="Threonine betaine?",
          compound_name_old=compound_name, formula="C7H15NO3", rt = 5,
          mz=162.1125, ionization_form="[M+H]", charge=1, 
          date_added=150101, polarity="pos") %>%
  add_row(compound_type="Custom", compound_name="Theanine?",
          compound_name_old=compound_name, formula="C7H14N2O3", rt = 6.8,
          mz=175.10771, ionization_form="[M+H]", charge=1, 
          date_added=150101, polarity="pos") %>%
  add_row(compound_type="Custom", compound_name="Asterina-330",
          compound_name_old=compound_name, formula="C12H20N2O6", rt = 10.5,
          mz=289.1393, ionization_form="[M+H]", charge=1, polarity="pos") %>%
  add_row(compound_type="Custom", compound_name="Shinorine",
        compound_name_old=compound_name, formula="C13H20N2O8", rt = 11.6,
        mz=333.1292, ionization_form="[M+H]", charge=1, polarity="pos") %>%
  add_row(compound_type="Custom", compound_name="Palythine",
        compound_name_old=compound_name, formula="C10H16N2O5", rt = 11.0,
        mz=245.1132, ionization_form="[M+H]", charge=1, polarity="pos") %>%
  add_row(compound_type="Custom", compound_name="Porphyra-334?",
        compound_name_old=compound_name, formula="C14H22N2O8", rt = 8.1,
        mz=347.1449, ionization_form="[M+H]", charge=1, polarity="pos") %>%
  add_row(compound_type="Custom", compound_name="Mycosporine-2-glycine?",
        compound_name_old=compound_name, formula="C12H18N2O7", rt = 11.3,
        mz=303.1186, ionization_form="[M+H]", charge=1, polarity="pos") %>%
  add_row(compound_type="Custom", compound_name="Palythene/Usujirene?",
        compound_name_old=compound_name, formula="C13H20N2O5", rt = 8.3,
        mz=285.1445, ionization_form="[M+H]", charge=1, polarity="pos") %>%
  add_row(compound_type="Custom", compound_name="Also Palythene/Usujirene?",
        compound_name_old=compound_name, formula="C13H20N2O5", rt = 6.9,
        mz=285.1445, ionization_form="[M+H]", charge=1, polarity="pos") %>%
  add_row(compound_type="Custom", compound_name="Mycosporine-glycine?",
        compound_name_old=compound_name, formula="C10H15NO6", rt = 10,
        mz=246.0972, ionization_form="[M+H]", charge=1, polarity="pos") %>%
  add_row(compound_type="Custom", compound_name="Also mycosporine-glycine?",
        compound_name_old=compound_name, formula="C10H15NO6", rt = 9.5,
        mz=246.0972, ionization_form="[M+H]", charge=1, polarity="pos") %>%
...
culture_desc <- readxl::read_xlsx("raw_data/Heal_mSystems uncorrected Metabs_SuppTabs_20210223.xlsx",
                  sheet = "S7_Culturedescriptions_full")
culture_data <- readxl::read_xlsx("raw_data/Heal_mSystems uncorrected Metabs_SuppTabs_20210223.xlsx",
...
write.csv(taxa_metabs, file = "made_data/taxa_metabs.csv")
```
