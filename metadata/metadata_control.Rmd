...
clean_stans <- "https://raw.githubusercontent.com/IngallsLabUW/" %>%
  paste0("Ingalls_Standards/master/Ingalls_Lab_Standards.csv") %>%
  read_csv() %>%
  filter(Column=="HILIC") %>%
  ...
  select(compound=Compound_Name, compound_type=Compound_Type, 
         formula=Empirical_Formula, stan_mz=mz, stan_rt=RT_minute, 
         mix=HILIC_Mix, conc=Concentration_uM, Date_Added, 
         name_old=Compound_Name_Original, polarity) %>%
  mutate(compound=case_when(
    compound=="Ethyl aminobenzoate"~"Ethyl aminobenzoate?",
    TRUE~compound
  ))
