...
We found that the metabolome clearly differed between the cyclonic and anticyclonic samples when all molecular features were analyzed. Multivariate statistics detected the largest differences in particulate matter composition at the DCM and 175 meter samples (Figure 1). At the DCM, approximately 16% of the variation in the metabolome could be explained by sea level anomaly (SLA) alone (PERMANOVA, p = 0.003) and SLA was strongly correlated with the first principal component (r = .615, % variance explained by PC1 = 27.2), indicating that this was one of the largest sources of variation in the dataset. Notably, the samples taken from the exact center of the cyclonic eddy were highly distinct and likely drove much of the variance explained. In the 175m samples, 12% of the variance was explained by SLA (PERMANOVA, p = 0.008) and SLA was highly correlated with the second PC of the dataset (r = .697, % variance explained by PC2 = 15.4), though the first PC did not seem to have any visible pattern with metadata and appeared to largely capture variation between biological triplicates (Supplemental figure 1). At the surface, SLA trends are less evident with a larger p-value (PERMANOVA, p=0.087) and a lower R$^2$ (0.09) (Figure 1).

To characterize the difference between eddies in a lower-dimensional space, we plotted an "average" metabolite peak area per station at each depth by taking the median value of each of the detected mass features after z-score normalization (Figure 1). As expected, the average metabolite collected at the exact center of the cyclonic eddy (Station 12) was more abundant than in other samples at this same depth. Similarly, the 175 meter samples showed the largest metabolite abundances in the anticyclone relative to other stations at this depth. This method also allowed us to see the remarkably sinusoidal response of the average metabolite across the stations. This response correlated negatively with SLA at the DCM (lower SLA = higher average values, $r_{DCM}= -.716$) but positively with SLA at the surface and at depth ($r_{surface} = .211$, $r_{deep} = .830$).
...
We expected a large part of the SLA trends to be due to the elevated biomass in the cyclone DCM relative to the anticyclone and the inverse pattern in the 175 meter samples. While we were unable to quantify a majority of the molecular features in the dataset for direct comparison to biomass metrics, a subset of those were known metabolites that we quantified using standards (Figure 2).
...
*Table 1: Detected mass features that were enriched in the 15m sample from Station 11 (MS11C215m_A). RT = retention time in minutes. Taurine betaine is followed by a ? to denote that this identification is putative. Feature ID is a disambiguating key produced by XCMS processing.*
...
In addition to absolute shifts in metabolite concentration due to biomass differences, we also explored compositional shifts in the metabolomes by assessing the fraction of the total metabolite pool contributed by each compound across the eddy transect.
...
To provide context for the sea level anomaly responses detected above, we additionally performed high-resolution depth sampling around the DCM at the center of both eddy poles. Here, we found that the ~40 meter depth range and the transition between the eddies explained similar amounts of variation in the data (R$^2_{depth}=0.253$, R$^2_{SLA}=0.238$) and were both highly significant factors (permutational p-values << 0.001). Additionally, both sample depth and SLA were correlated with the first principal component of the metabolite matrix (r$_{depth}=.766$, r$_{SLA}=.684$, fraction of variance explained by PC$_1$ = 30.8%) (Figure 3). A 2D NMDS plot of the samples showed clear separation between the eddies, with cyclonic samples largely on the left side of Figure 3 while anticyclonic samples fell almost entirely to the right. Notably, the cyclonic samples show a much larger spread than the anticyclonic ones, indicating larger sample variability at the surface than the DCM. A depth gradient is visible along MDS 2, with the deepest samples generally at the top right of the plot and the shallowest ones closer to the bottom and left.
...
MC_nmds_gp <- multivar_analysis_MC$nmds_output %>%
  ...
  geom_vline(xintercept = 0) +
  geom_hline(yintercept = 0) +
  geom_point(aes(x=MDS1, y=MDS2, color=sla_class), size=5, pch=19) +
  scale_color_manual(
  ...
  geom_point(aes(x=MDS1, y=MDS2, fill=depth, alpha=depth), size=5, pch=21, color="black") +
  scale_fill_manual(
    values = c("white", "white", "white", "black", "black"),
    breaks = c("-20m", "-10m", "DCM", "+10m", "+20m")
  ) +
  scale_alpha_manual(
    values = c(0.7, 0.4, 0.2, 0.1, 0.4),
    breaks = c("-20m", "-10m", "DCM", "+10m", "+20m")
  ) +
  coord_fixed() +
...
Several molecule- or pathway-specific narratives emerged from the metabolomic data. The untargeted approach used here allowed us to describe and characterize signals from small molecules whose identity was unknown, a particularly promising approach in open ocean gyres where the largest fraction of unknowns is found [@Heal2021]. We found that our list of authentic standards covered fewer metabolites in anticyclonic eddies at all depths, with the molecular diversity of the cyclone much better characterized. Many of the trends detected below persisted even after normalizing within each sample, indicating that the shifts discussed have implications beyond simple scaling with biomass.
...
## Cruise information
Samples were collected from two cruises in the North Pacific Subtropical Gyre near Station ALOHA that targeted strong mesoscale eddy features as described in @Dugenne2023 and @Gleich2024. Briefly, the 2017 MESOSCOPE cruise (Microbial Ecology of the Surface Ocean-Simons Collaboration on Ocean Processes and Ecology, KM1709 on the *R/V Kilo Moana*) consisted of a transect across the cyclonic and anticyclonic poles of the eddy dipole as well as two long-term Lagrangian stations at the center of each eddy. The cyclonic pole of the eddy had a maximum negative SLA anomaly of -20 centimeters and the anticyclonic center reached +24 centimeters. The transect samples were taken at various times of day as the ship transected the dipole, and the eddy center samples were all collected between 5 and 8 pm. In 2018, the Falkor cruise (FK180310 on the *R/V Falkor*) targeted a new eddy dipole in approximately the same location with samples taken at the center of each eddy (maximum negative anomaly in the cyclone = -15 cm, maximum positive anomaly in the anticyclone = 26 cm).
...
This study reports how changes in the biogeochemical environment and its associated community due to mesoscale eddy-driven mechanisms affect the metabolome and thereby the composition of particulate organic matter. The transect across an oligotrophic eddy dipole shows that many metabolites track closely with metrics of overall biomass, with eddy effects stronger at 175 meters than at the surface or deep chlorophyll maximum (DCM). High-resolution depth sampling of the DCM at the center of each eddy elucidated several known and unknown biomarkers likely corresponding to the well-documented increase in picoeukaryotes in the cyclone. Metabolite clusters aligned well with expected trends in abundance with depth and between the two eddies, with rapid convergence below the DCM towards a general deep-sea metabolite signal. By contrasting these effects with a followup analysis in the centers of a separate dipole the following year, we also learned that the importance of eddies may be an unpredictable interaction between origin and age. Finally, the metabolites with the strongest responses to eddy effects were unknown and a relatively smaller proportion of known metabolites in the anticyclone demonstrates the utility of untargeted metabolomics in exploring environmental variation and the large amount of information that can still be gleaned from such datasets.
