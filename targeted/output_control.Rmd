...
clean_stans <- read.csv("../metadata/made_data/clean_stans.csv")
name_conversion <- clean_stans %>%
  select(new=compound_name, old=compound_name_old) %>%
  distinct()
file_metadata <- read.csv("../metadata/made_data/file_metadata.csv")
```
...
```{r MSDIAL autoexport things not yet}
# Not implemented yet, check on thread below for updates
# http://www.metabolomics-forum.com/index.php?topic=1820.0
# Instead, open an existing .mtd2 file, click the "Export" tab, select "Alignment result", specify the output directory as "Area_HILICPos_EddyTransect.csv", check a couple of the boxes (Raw Data Area is the only critical one), change the output format to msp(?), and hit the "Export" button at the bottom. You then need to open this file in Excel and save as CSV.

# Haven't been able to find the raw data for negative mode so I'm currently just copying this off the Google Drive (G:\Shared drives\Ingalls Lab\Collaborative Projects\MESO-SCOPE\EddyTransect\HILIC_Neg_MSDial)
```
...
```{r transectSkylineImport}
manual_pos <- read_csv("raw_data/Mesotransect/Area_HILICPos_EddyTransect.csv", skip = 4) %>%
  ...
  mutate(polarity="pos")
manual_neg <- read_csv("raw_data/Mesotransect/Area_HILICNeg_EddyTransect.csv", skip = 4) %>%
  filter(`Metabolite name`!="Unknown") %>%
  select(compound_name=`Metabolite name`, starts_with("180821")) %>%
  pivot_longer(cols=starts_with("180821"), names_to = "filename", values_to="area") %>%
  mutate(polarity="neg")
...
transect_stans <- clean_stans %>%
  filter(date_added<=transect_rundate)

rfs_transect <- transect_stans %>%
  left_join(clean_ints_transect, by="compound_name") %>%
  filter(str_detect(filename, "_Std_")) %>% 
...
mesotransect_data <- rfs_transect %>% 
  left_join(BMISed_transect, by="compound_name") %>%
  left_join(clean_stans) %>%
...
# Instead, use rf_ratio from Falkor to swap in
center_rundate <- strptime("180205", "%y%m%d")
center_stans <- clean_stans %>%
  filter(date_added<=center_rundate)

rfs_center <- center_stans %>% 
  left_join(clean_ints_center, by="compound_name") %>% 
  filter(str_detect(filename, "_Std_")) %>%
  filter(compound_type!="Internal Standard") %>%
  select(-compound_type) %>% 
  ...
  left_join(rfs_falkor %>% select("rf_ratio", "compound_name"), by="compound_name")
...
raw_area_data <- rbind(
  ...
  left_join(clean_stans, by="compound_name")
...
file_metadata %>%
  ...
  distinct(filename, samp_type, station, cast, depth, abs_depth, rosette,date_run, timestamp) %>%
  rename(sample_name=filename, file_timestamp=timestamp) %>%
  writeData(wb = wb, sheet = "File metadata")
addWorksheet(wb = wb, sheetName = "Compound metadata")
clean_stans %>%
  select(compound_name, compound_type, formula, rt, mz, kegg_id, polarity, peak_quality) %>%
...
data.frame(`README for Ingalls MESOSCOPE data`=c(
  ...
  "file_timestamp = The time at which the sample was actually run",
  "",
  ...
  "rt = Estimated retention time of the compound, in minutes",
  "mz = Mass-to-charge ratio of the molecule at which it's detected in the mass spec",
